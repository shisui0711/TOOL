...
def format_print(symbol, text):
        return f"""{Col.Symbol(symbol, Col.green, Col.green)} {Col.green}{text}{Col.reset}"""
def format_print_error(symbol, text):
        return f"""{Col.Symbol(symbol, Col.red, Col.red)} {Col.red}{text}{Col.reset}"""
def format_input(symbol, text):
        return f"""{Col.Symbol(symbol, Col.green, Col.blue)} {Col.green}{text}{Col.reset}"""
def banner():
...
banner()
dem = 0
ck = input (format_input('</>','Nhập Cookie Acc Reg Pro5 : '))
dl =input(format_input('</>','Nhập Delay Reg Pro5: '))
def delay(dl):
...
while True:
    ...
    name = str(ho+' '+lot+' '+ten)
    dem = dem+1
    print(format_print(dem,reg_pro5(cookies, name).Reg()))#kết quả trả về của api
    delay(dl);
